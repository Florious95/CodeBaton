...
use codebaton_core::{AisyncError, DeviceId, DeviceInfo, Direction, OsType, Result};
use codebaton_sync::{
...
use codebaton_transport::{generate_tls_identity, ReceiveService, TcpTransporter, TlsConfig};
use clap::{Parser, Subcommand};
use indicatif::{ProgressBar, ProgressStyle};

#[derive(Debug, Parser)]
#[command(name = "codebaton", version, about = "AI session and project sync")]
struct Cli {
...
fn run_transfer(
...
) -> Result<codebaton_sync::SyncReport> {
    if matches!(direction, TransferDirection::Pull) {
        return Err(AisyncError::Transport(
            "pull over TCP requires a remote control channel; start `codebaton serve --dir <local-target>` locally and run `codebaton send` on the peer".to_string(),
        ));
    }
    ...
    let endpoint = peer.endpoint.ok_or_else(|| {
        AisyncError::Config(format!(
            "peer '{peer_name}' has no endpoint; run `codebaton pair {peer_name} --endpoint <ip:port>`"
        ))
    })?;
...
) -> Result<codebaton_core::SyncManifest> {
    if !source.is_dir() {
        return Err(AisyncError::InvalidInput(format!(
            "source is not a directory: {}",
            source.display()
...
fn print_report(label: &str, report: &codebaton_sync::SyncReport) {
    println!(
...
mod tests {
    ...
    fn clap_accepts_all_required_commands() {
        ...
        for args in [
            ["codebaton", "devices"].as_slice(),
            ["codebaton", "pair", "desktop"].as_slice(),
            ["codebaton", "unpair", "desktop"].as_slice(),
            ["codebaton", "projects"].as_slice(),
            ["codebaton", "add", "/tmp/a", "/tmp/b"].as_slice(),
            ["codebaton", "workspace", "/tmp/a", "/tmp/b"].as_slice(),
            ["codebaton", "push", "app", "--to", "desktop"].as_slice(),
            ["codebaton", "pull", "app", "--from", "desktop"].as_slice(),
            [
                "codebaton",
                "serve",
                "--port",
            ...
            .as_slice(),
            ["codebaton", "sync", "app", "--auto"].as_slice(),
            ["codebaton", "status"].as_slice(),
            ["codebaton", "log", "--project", "app"].as_slice(),
        ] {

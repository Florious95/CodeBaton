...
use aisync_app_lib::backend::{Backend, SplitBrainStatus};
use aisync_core::{Direction, OsType, Result};
use aisync_sync::{ClaudeConfig, DeviceConfig, SyncConfig, SyncModeConfig, SyncSnapshot};
use aisync_transport::{manifest_hash, scan_manifest};
use uuid::Uuid;
...
fn io_err(e: std::io::Error) -> aisync_core::AisyncError {
    aisync_core::AisyncError::Transport(e.to_string())
}
...
impl TwoBackendBuilder {
    ...
    pub fn build(self) -> TwoBackend {
        ...
        backend_a
            .add_peer_endpoint(
                peer_name.clone(),
                aisync_core::DeviceId(Uuid::new_v4()),
...
impl WorkspaceHarness {
    ...
    /// A → B 工作区同步（推送整棵子目录树）。
    pub fn sync(&self) -> Result<aisync_sync::SyncReport> {
        self.a
...
impl TwoBackend {
    ...
    pub fn push(&self, confirm_overwrite: bool) -> Result<aisync_sync::SyncReport> {
        self.a.run_sync(
            &self.project_name,
            &self.peer_name,
    ...
    pub fn a_snapshot(&self) -> Option<SyncSnapshot> {
        let cfg = aisync_sync::load_config(&self.a_config_path).ok()?;
    ...
    pub fn b_serve(&self) -> aisync_app_lib::backend::ServeInfo {
    ...
    pub fn event_count_for_project(&self, event: &str) -> u64 {
        aisync_app_lib::backend::event_count(&format!("{event}:{}", self.project_name))
    }
...
fn bare_config(device: &str, receive_port: u16, root: &Path) -> SyncConfig {
    SyncConfig {
        device: DeviceConfig {
            id: aisync_core::DeviceId(Uuid::new_v4()),
            name: device.into(),
        },
        onboarded: false,
        peers: HashMap::new(),
...
fn dir_hash(dir: &Path) -> String {
    manifest_hash(&scan_manifest(dir).unwrap_or(aisync_core::SyncManifest { files: vec![] }))
}
...
pub fn assert_safety_valve_aborted(
    result: &Result<aisync_sync::SyncReport>,
) -> std::result::Result<(), String> {

...
use codebaton_core::DeviceId;
use codebaton_core::{Direction, OsType};
use codebaton_sync::{
    ClaudeConfig, DeviceConfig, PeerConfig, ProjectConfig, SyncConfig, SyncModeConfig,
};
use codebaton_transport::{generate_tls_identity, ReceiveService, TlsConfig};
use uuid::Uuid;
...
fn confirm_pairing_persists_config_peer_when_offline() {
    ...
    let reloaded = codebaton_sync::load_config(&config_path).unwrap();
    assert!(
...
fn add_workspace_persists_entity_and_syncs_whole_root_tree() {
    ...
    let persisted = codebaton_sync::load_config(&config_path).unwrap();
    assert!(
    ...
    let persisted_b = codebaton_sync::load_config(&b_config_path).unwrap();
...
struct ReceiveThread {
    cert_der: Vec<u8>,
    handle: thread::JoinHandle<codebaton_core::Result<()>>,
}

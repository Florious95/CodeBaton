...
//! are routed through the real [`Backend`] (codebaton-sync / codebaton-discovery /
//! codebaton-transport). Display data that the backend does not yet track
...
fn project_dto_from_config(p: &codebaton_sync::ProjectConfig) -> ProjectDto {
    ...
    ProjectDto {
        ...
        mode: match p.sync_mode {
            codebaton_sync::SyncModeConfig::OneWayPush => SyncModeDto::OneWayPush,
            codebaton_sync::SyncModeConfig::OneWayPull => SyncModeDto::OneWayPull,
            codebaton_sync::SyncModeConfig::TwoWayAuto => SyncModeDto::TwoWayAuto,
        },
...
fn workspace_dto_from_config(w: &codebaton_sync::WorkspaceConfig, backend: &Backend) -> WorkspaceDto {
...
fn os_str(os: &codebaton_core::OsType) -> String {
    match os {
        codebaton_core::OsType::Darwin => "darwin".into(),
        codebaton_core::OsType::Windows => "windows".into(),
        codebaton_core::OsType::Linux => "linux".into(),
        codebaton_core::OsType::Other(s) => s.clone(),
    }
}

fn os_display(os: &codebaton_core::OsType) -> String {
    match os {
        // Include the real macOS version (e.g. "macOS 15.5") like the prototype.
        codebaton_core::OsType::Darwin => match macos_product_version() {
            Some(v) => format!("macOS {v}"),
            None => "macOS".into(),
        },
        codebaton_core::OsType::Windows => "Windows".into(),
        codebaton_core::OsType::Linux => "Linux".into(),
        codebaton_core::OsType::Other(s) if !s.is_empty() => s.clone(),
        codebaton_core::OsType::Other(_) => std::env::consts::OS.to_string(),
    }
}
...
fn workspace_history_scope(
    config: &codebaton_sync::SyncConfig,
    project_id: &str,
) -> (
    Option<String>,
    Option<String>,
...
pub fn begin_pairing(
    ...
    let pairing = backend
        .pairing_code(&codebaton_core::DeviceId(uuid))
        .map_err(|e| {
...
pub fn confirm_pairing(
    ...
    backend
        .confirm_pairing(&codebaton_core::DeviceId(uuid))
        .map_err(|e| {
            crate::backend::log_line(&format!(
...
pub fn unpair(state: State<AppState>, backend: State<Backend>, app: AppHandle, peer_id: String) {
    if let Ok(uuid) = peer_id.parse::<uuid::Uuid>() {
        let _ = backend.unpair(&codebaton_core::DeviceId(uuid));
    }
    let _ = tray::refresh(&app, &state, &backend);
}
...
//
// Asks the peer (over the control channel) whether the project's remote target
// directory already contains files. The UI calls this before a push and, if
// true, shows an overwrite-confirm dialog before starting the sync.
//
// Placeholder: returns false until session implements the control-channel probe
// in `backend`. Returning false means "empty" → push proceeds without prompting,
// preserving today's behaviour until the real check lands.
#[tauri::command]
pub fn check_target_not_empty(
    backend: State<Backend>,
    project_id: String,
    peer_name: String,
) -> std::result::Result<bool, String> {
    backend
        .check_target_not_empty(&project_id, &peer_name)
        .map_err(|e| e.to_string())
}
...
pub fn start_sync(
    ...
    // peer's original target dir is fully backed up to a `<name>.bak-<timestamp>`
    // sibling (kept, not deleted) before the incremental merge.
    let confirm_overwrite = confirm_overwrite.unwrap_or(false);
...
pub fn add_peer_endpoint(
    ...
    let id = peer_id
        ...
        .map(codebaton_core::DeviceId)
        .unwrap_or_else(codebaton_core::DeviceId::new);

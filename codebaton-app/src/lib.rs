...
pub fn run() {
    ...
    backend::log_line("[codebaton-app] starting — logs at ~/.aisync/logs/aisync.log");
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            if let Some(win) = app.get_webview_window("main") {
                ...
                let _ = win.set_focus();
                eprintln!(
                    "[codebaton-app] main window ready: visible={:?} url={:?}",
                    win.is_visible(),
                    win.url().map(|u| u.to_string())
                );
            } else {
                eprintln!("[codebaton-app] WARNING: main window not found at setup");
            }
            Ok(())
        })

...
use codebaton_session::{ClaudeCodeParser, ParsedSession, PathRule, RuleBasedRewriter, SessionIndex};
use codebaton_sync::{SyncConfig, SyncCoordinator, SyncModeConfig, WorkspaceConfig};
...
fn scenario_04_structured_path_fields_are_rewritten() {
    ...
    let report = ClaudeCodeParser::rewrite_structured_paths(
        &mut sessions[0],
        &rewriter,
        codebaton_core::RewriteDirection::SourceToTarget,
    );
...
fn scenario_05_path_rewrite_round_trip_is_reversible() {
    ...
    let rewritten = codebaton_core::PathRewriter::rewrite(
        &rewriter,
        before,
        codebaton_core::RewriteDirection::SourceToTarget,
    )
    .unwrap();
    let round_trip = codebaton_core::PathRewriter::rewrite(
        &rewriter,
        &rewritten,
        codebaton_core::RewriteDirection::TargetToSource,
    )
    .unwrap();
...
impl SessionParser for NoSessionParser {
    ...
    fn rewrite_paths(
        ...
        _rewriter: &dyn codebaton_core::PathRewriter,
    ) -> Result<()> {
        Ok(())
    }

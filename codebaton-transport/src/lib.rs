...
use codebaton_core::{AisyncError, DeviceInfo, FileEntry, Result, SyncManifest, Transporter};
...
fn server_device_info() -> DeviceInfo {
    DeviceInfo {
        id: codebaton_core::DeviceId::new(),
        name: "aisync-server".to_string(),
        os: codebaton_core::OsType::Other(std::env::consts::OS.to_string()),
        addresses: Vec::new(),
        protocol_version: PROTOCOL_VERSION,
    }
}
...
fn trace_stage(stage: &str, detail: impl std::fmt::Display) {
    eprintln!("[codebaton-transport] stage={stage} {detail}");
}
    ...
    fn control_frames_round_trip_for_messages_and_file_transfer() {
        ...
        runtime.block_on(async {
            let device = DeviceInfo {
                id: codebaton_core::DeviceId::new(),
                name: "peer".to_string(),
                os: codebaton_core::OsType::Darwin,
                addresses: Vec::new(),
                protocol_version: PROTOCOL_VERSION,
            };
            let messages = vec![
